"""
main.py — Full pipeline orchestrator for the ArvyaX ML Internship Assignment.
Runs all steps end-to-end: data loading, feature engineering, model training,
ablation study, prediction, error analysis, and summary metrics.
"""
...
| Mean confidence on errors | {mean_conf_errors} |
| Mean confidence on correct | {mean_conf_correct} |
...
"""
    ...
    classes = le_state.classes_ if le_state is not None else np.unique(y_true)
...
def print_summary(model_results, predictions):
    ...
    print(f"    Best model:    {model_results['state_metrics']['best_model_name']}")
    print(f"    XGB CV F1:     {model_results['state_metrics']['xgb_cv_f1']:.4f}")
    print(f"    RF CV F1:      {model_results['state_metrics']['rf_cv_f1']:.4f}")

    print(f"\n  Intensity Model:")
    print(f"    Approach:      {model_results['intensity_approach']}")
    print(f"    RMSE:          {model_results['intensity_metrics']['rmse']:.4f}")
    print(f"    Accuracy:      {model_results['intensity_metrics']['accuracy']:.4f}")

    print(f"\n  Predictions:")
    print(f"    Total:         {len(predictions)}")
    print(f"    Mean conf:     {predictions['confidence'].mean():.4f}")
    print(f"    Uncertain:     {predictions['uncertain_flag'].sum()} ({predictions['uncertain_flag'].mean()*100:.1f}%)")

    print(f"\n  Action Distribution:")
    for action, count in predictions['what_to_do'].value_counts().items():
        print(f"    {action:<20} {count:>4}")

    print(f"\n  Timing Distribution:")
    for timing, count in predictions['when_to_do'].value_counts().items():
        print(f"    {timing:<20} {count:>4}")


def main():
    """Run the complete ArvyaX ML pipeline."""
    start_time = time.time()

    print("\n" + "█" * 60)
    print("  ArvyaX — Emotional Intelligence ML Pipeline")
    print("  ML Internship Assignment")
    print("█" * 60)
    ...
    # ---- STEP 3: Train Models ----
    model_results = train_all_models(features['X_combined_train'], train_df, tfidf=features['tfidf'])

    # ---- STEP 6: Ablation Study ----
    ablation_results = run_ablation(
        features['X_text_only_train'],
        features['X_combined_train'],
        train_df['emotional_state']
    )
    ...
    run_error_analysis(train_df, features['X_combined_train'], model_results)

    # ---- Summary ----
    print_summary(model_results, predictions)

    elapsed = time.time() - start_time
    print(f"\n  ⏱ Total pipeline time: {elapsed:.1f} seconds")
    print(f"\n  Output files:")
    print(f"    • predictions.csv")
    print(f"    • saved_artifacts/ (models, encoders)")
    print(f"    • ERROR_ANALYSIS.md")
    print(f"\n" + "█" * 60)
